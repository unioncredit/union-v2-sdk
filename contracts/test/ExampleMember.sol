...
contract ExampleMember is Ownable, UnionVoucher, UnionBorrower{
    ...
    function updateTrust(address account, uint96 amount) public onlyOwner {
    ...
    function stake(uint96 amount) public onlyOwner {
        underlyingToken.transferFrom(msg.sender, address(this), amount);
        _stake(amount);
    }

    function unstake(uint96 amount) public onlyOwner {
        _unstake(amount);
